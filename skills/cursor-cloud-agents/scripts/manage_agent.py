...
def parse_args():
    parser = argparse.ArgumentParser(description="Stop or delete a Cursor Cloud Agent")
    ...
    parser.add_argument(
        "--force", action="store_true", help="Skip confirmation for delete"
    )
...
def manage_agent(args):
    ...
    if args.action == "delete" and not args.force:
        confirm = input(
            f"⚠️  Are you sure you want to DELETE agent {args.agent_id}? This cannot be undone. (y/N): "
        )
        if confirm.lower() != "y":
            print("Cancelled.")
            sys.exit(0)
