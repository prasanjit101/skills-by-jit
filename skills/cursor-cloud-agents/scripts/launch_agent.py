...
def parse_args():
    parser = argparse.ArgumentParser(description="Launch a Cursor Cloud Agent")
    ...
    parser.add_argument(
        "--prompt", type=str, required=True, help="Instruction text for the agent"
    )
    ...
    parser.add_argument(
        "--pr-url", type=str, help="PR URL to work on (overrides --repo and --ref)"
    )
    ...
    parser.add_argument(
        "--branch-name", type=str, help="Custom branch name for the agent"
    )
    parser.add_argument(
        "--open-as-cursor-app",
        action="store_true",
    ...
    # Webhook
    parser.add_argument(
        "--webhook-url", type=str, help="Webhook URL for status notifications"
    )
    parser.add_argument(
        "--webhook-secret",
        type=str,
    ...
    # Image (optional)
    parser.add_argument(
        "--image", type=str, help="Path to image file to include (base64 encoded)"
    )

    # Output
    parser.add_argument("--json", action="store_true", help="Output raw JSON response")
    parser.add_argument(
        "--quiet", action="store_true", help="Minimal output (only agent ID and URL)"
    )
...
def encode_image(image_path: str) -> dict:
    ...
    return {"data": data, "dimension": {"width": width, "height": height}}
...
def launch_agent(args):
    ...
    # Build request body
    body = {"prompt": {"text": args.prompt}, "source": {}}
    ...
    if any(
        ...
        if args.skip_reviewer:
            if not (args.auto_create_pr and args.open_as_cursor_app):
                print(
                    "Warning: --skip-reviewer requires --auto-create-pr and --open-as-cursor-app"
                )
