...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check Cursor Cloud Agent status"
    )
    ...
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="Minimal output (only status)"
    )
            ...
            if args.conversation:
                ...
                if args.quiet:
                    ...
                    if result.get('source'):
                        print("Source:")
                        print(f"  Repository: {result['source'].get('repository', 'N/A')}")
                        if result['source'].get('ref'):
                            print(f"  Ref:        {result['source']['ref']}")
                    
                    if result.get('target'):
                        print("\nTarget:")
                        print(f"  Branch:     {result['target'].get('branchName', 'N/A')}")
                        print(f"  URL:        {result['target'].get('url', 'N/A')}")
                        if result['target'].get('prUrl'):
                            print(f"  PR:         {result['target']['prUrl']}")
                        print(f"  Auto PR:    {result['target'].get('autoCreatePr', False)}")
